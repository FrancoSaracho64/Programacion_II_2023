...
void ejercicio2D(){
    ...
    printf("-------------------------------------------------------------------------------------------------\nCOMPLEJIDAD ALGORITMICA: \n");
    printf(" La  complejidad  algoritmica  de las funciones  A, B y C, en  el peor  de los casos, es del tipo\n O(n  log(2) n), ya que  se  multiplica  n  (que  es  la cantidad de niveles que tiene el  arbol)\n y log(2)  n  porque  por cada llamada  recursiva, el problema  se  divide  en  dos (log(2)) y el\n tiempo de cada nivel es n.\n Esto nos daria como resultado final:  O(n  log(2) n).");
    printf("\n-------------------------------------------------------------------------------------------------\n\n\n");
}

/*Ejercicio 3 ---------------------------------------------------------------------------------- */
NodoArbol ejercicio3(NodoArbol nodo, int numero){
    //Primero nos damos la seguridad de haber encontrado el nodo ingresado por el usuario
    bool encontrado = false;
    NodoArbol nodo_n = n_crear(NULL);
    nodoEncontrado(nodo, numero, &encontrado, &nodo_n);      
    if (encontrado){
        /*a*/
        NodoArbol resultado_a;
        nodo_padre(nodo, numero, &resultado_a);
        if (resultado_a == NULL) 
            printf("\n\n  ---A)   El nodo ingresado es la RAIZ del arbol. No tiene padre.\n\n");
        else 
        ...
        printf("  ---B)   Los hijos del nodo ingresado son:\n");
        ...
        printf("  ---C)   El hermano del nodo ingresado es:\n");
        ...
        printf("          Los nodos del nivel ingresado son:\n");
        l_mostrarLista(res_f);
    } else {
        printf("\nNO se ha encontrado el nodo cargado.\nPor lo tanto, no se podran ejecutar las funciones de este ejercicio.");
    }
}

void nodoEncontrado(NodoArbol nodo, int numero, bool *encontrado, NodoArbol *nodoEs){
    ...
    if(!a_es_rama_nula(n_hijoizquierdo(nodo)))
        nodoEncontrado(n_hijoizquierdo(nodo), numero, encontrado, nodoEs);
    if(!a_es_rama_nula(n_hijoderecho(nodo)))
        nodoEncontrado(n_hijoderecho(nodo), numero, encontrado, nodoEs);    
}

//a
void nodo_padre(NodoArbol nodo, int numero, NodoArbol *x){
    if (!a_es_rama_nula(nodo)){
        if(n_recuperar(nodo)->clave == numero) {
            *x = NULL;
            return;
        }
...
Lista nodos_mismoNivel_inic(NodoArbol nodo){
    ...
    do {
        do {
            fflush(stdin);
            printf("  ---F)   Ingrese el nivel del arbol que quiere mirar: ");
...
void BinarioAvlInt (ArbolAVL retorno, NodoArbol q) {
   TipoElemento x;
    ...
    if (q == NULL) {
      //caso base, como la funcion es void no tiene ninguna tarea
    }
    else {
         x = n_recuperar(q);

         //paso el elemento actual al avl         
         avl_insertar(retorno, x);
        
         // Llamo para el recorrido
         BinarioAvlInt(retorno, n_hijoizquierdo(q));
         BinarioAvlInt(retorno, n_hijoderecho(q));
    }
}

void BinarioAvl (ArbolAVL retorno, ArbolBinario a) {
   BinarioAvlInt(retorno, a_raiz(a));
}
